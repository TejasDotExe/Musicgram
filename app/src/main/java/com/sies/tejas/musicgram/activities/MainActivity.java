...
import com.sies.tejas.musicgram.adapters.ActivityMainAlbumItemAdapter;
import com.sies.tejas.musicgram.adapters.ActivityMainArtistsItemAdapter;
import com.sies.tejas.musicgram.adapters.ActivityMainPlaylistAdapter;
import com.sies.tejas.musicgram.adapters.ActivityMainPopularSongs;
import com.sies.tejas.musicgram.adapters.SavedLibrariesAdapter;
import com.sies.tejas.musicgram.databinding.ActivityMainBinding;
...
import com.sies.tejas.musicgram.utils.KMeansClustering;
import com.sies.tejas.musicgram.utils.NetworkUtil;
...
public class MainActivity extends AppCompatActivity {
    ...
    private void showData() {
        ...
        List<SongResponse.Song> topClusteredSongs = km.getTopClusteredSongs(ApplicationClass.sharedPreferenceManager.getPlayedSongs(), 3);

        if (!topClusteredSongs.isEmpty()) {
            String songId = topClusteredSongs.get(0).id(); // choose first or random song

            apiManager.retrieveSongSuggestions(songId,15, new RequestNetwork.RequestListener() {
                @Override
                public void onResponse(String tag, String response, HashMap<String, Object> responseHeaders) {
                    SongSearch songSearch = new Gson().fromJson(response, SongSearch.class);
                    Log.i(TAG, "onResponse: " + response);
